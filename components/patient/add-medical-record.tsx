...
import { addPatientMedicalRecord } from "@/lib/actions/medical-record-actions"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!currentUser) {
        toast({
          title: "Erreur",
          description: "Vous devez être connecté pour ajouter un dossier médical.",
          variant: "destructive",
        })
        return
      }
      ...
      const result = await addPatientMedicalRecord(formDataObj)

      if (result.success) {
        toast({
          title: "Dossier médical ajouté",
          description: "Votre dossier médical a été ajouté avec succès.",
        })
        ...
        addMedicalRecord({
          ...
          attachments: formData.attachments.map(file => file.name).join(","),
        })
        ...
        toast({
          title: "Erreur",
          description: result.message || "Une erreur s'est produite lors de l'ajout du dossier médical.",
          variant: "destructive",
        })
      }
    } catch (error) {
      ...
      toast({
        title: "Erreur",
        description: "Une erreur s'est produite lors de l'ajout du dossier médical.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm" data-add-record>
      ...
      <DialogContent className="sm:max-w-[600px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              Ajoutez une nouvelle entrée à votre dossier médical. Ces informations seront visibles par vos médecins.
            </DialogDescription>
          </DialogHeader>
