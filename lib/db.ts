...
async function seedInitialData(db: IDBPDatabase<SanteTogoDBSchema>) {
  ...
  for (const patient of patients) {
    await patientTx.store.add(patient)
  }
  ...
  const appointmentTx = db.transaction("appointments", "readwrite")
  for (const appointment of appointments) {
    await appointmentTx.store.add(appointment)
  }
  await appointmentTx.done
  ...
  const medicationTx = db.transaction("medications", "readwrite")
  for (const medication of medications) {
    await medicationTx.store.add(medication)
  }
  await medicationTx.done
  ...
  const medicalRecordTx = db.transaction("medicalRecords", "readwrite")
  for (const record of medicalRecords) {
    await medicalRecordTx.store.add(record)
  }
  await medicalRecordTx.done
  ...
  const bloodDonationTx = db.transaction("bloodDonations", "readwrite")
  for (const donation of bloodDonations) {
    await bloodDonationTx.store.add(donation)
  }
  await bloodDonationTx.done
  ...
  const bloodRequestTx = db.transaction("bloodRequests", "readwrite")
  for (const request of bloodRequests) {
    await bloodRequestTx.store.add(request)
  }
  await bloodRequestTx.done
}
